...
class StockPortfolio:
    ...
    def refresh_prices(self):
        for i, symbol in enumerate(list(self.stocks.keys())):
            try:
                price = self.get_stock_price(symbol)
                self.stocks[symbol] = price
                ...
                self.message_label.config(text=f"Failed to refresh prices for {symbol}: {e}")
    ...
    def get_stock_price(self, symbol):
        ...
        try:
            ...
            if "Global Quote" in data and data["Global Quote"].get("05. price"):
                ...
                raise Exception("Invalid symbol or no data returned")
